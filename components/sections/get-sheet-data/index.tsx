...
const LeadCard: React.FC<{
    ...
    const handleWhatsAppClick = () => {
        window.open(createWhatsAppUrl(MESSAGE_TEMPLATES.singleLead(lead)), '_blank');
    };
    ...
    return (
        <div className="bg-secondary-950 text-center text-primary-50 aspect-square rounded-xl gap-4 flex flex-col shadow-md p-6">
            ...
            <div className="flex items-center justify-center gap-4 whitespace-nowrap">
                <Button className="text-primary-950 gap-2 w-fit" onClick={handleWhatsAppClick}>
                    <WhatsAppIcon />
...
export default function LeadManagement() {
    ...
    const composeSelectedLeadsMessage = useCallback(() => {
        ...
        if (selectedLeadsCodes.length) {
            window.open(createWhatsAppUrl(MESSAGE_TEMPLATES.multipleLeads(selectedLeadsCodes)), '_blank');
        }
    }, [leads, selectedLeads]);
        ...
        <Section className="bg-beige-primary rounded-3xl">
            <Container>
                ...
                <SelectField
                    ...
                    options={Object.keys(citiesByState).map(state => ({ value: state, label: state }))}
                ...
                {filters.state && (
                    <SelectField
                        ...
                        options={citiesByState[filters.state].map(city => ({ value: city, label: city }))}
